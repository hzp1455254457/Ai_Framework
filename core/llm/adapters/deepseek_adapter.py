...
class DeepSeekAdapter(BaseLLMAdapter):
    ...
    async def initialize(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not self._api_key:
            raise AdapterCallError("DeepSeek API密钥未配置")
        ...
        # 设置模型能力标签（DeepSeek模型能力）
        capability = ModelCapability(
            reasoning=True,  # DeepSeek擅长推理和代码生成
            creativity=True,
            cost_effective=True,  # DeepSeek成本相对较低
            fast=True,
            multilingual=True,
        ...
        # 设置成本信息（DeepSeek的示例成本，实际成本可能因模型而异）
        self.set_cost_per_1k_tokens(
            input_cost=0.00014,   # 示例成本（需要根据实际定价调整）
            output_cost=0.00028,   # 示例成本（需要根据实际定价调整）
        )
    ...
    async def call(
        ...
        **kwargs: Any,
        ...
        # 构建请求数据（DeepSeek使用OpenAI兼容的API格式）
        ...
        try:
            ...
            raise AdapterCallError(f"DeepSeek API调用失败: {e}") from e
        except Exception as e:
            raise AdapterCallError(f"DeepSeek API调用出错: {e}") from e
    ...
    async def stream_call(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise AdapterCallError(f"DeepSeek流式API调用失败: {e}") from e
        except Exception as e:
            raise AdapterCallError(f"DeepSeek流式API调用出错: {e}") from e
