...
- [x] **Resume路由** (P2) - `api/routes/resume.py`
  - 简历解析接口
  - 简历优化接口
  - 简历生成接口
  - 模板列表接口
  - 文件下载接口
  - 完成日期：2026-01-28
  - 说明：已实现 Resume API 路由，包含解析、优化、生成、模板管理等接口，支持PDF/Word/JSON格式解析，集成通义千问进行智能优化，支持HTML/PDF格式生成，编写了完整的单元测试
...
- [x] **简历功能页面** (P2) - `Ai_Web/src/views/Resume.vue` - 2026-01-28
  - Resume页面主组件
  - 简历上传组件（ResumeUpload.vue）
  - 简历预览组件（ResumePreview.vue）
  - 简历优化组件（ResumeOptimization.vue）
  - 模板选择组件（TemplateSelector.vue）
  - 简历导出组件（ResumeExport.vue）
  - Resume API客户端
  - Resume状态管理（Pinia store）
  - 完成日期：2026-01-28
  - 说明：已实现完整的简历功能前端页面，支持文件上传、解析、优化、模板选择、生成和导出，移除了Agent页面，聚焦核心功能（对话、图片识别、简历优化）
